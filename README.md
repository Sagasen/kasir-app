...
Versi React dari aplikasi kasir. Menu dibuat sederhana: **Kasir, Transaksi, dan Rekapitulasi** (khusus owner). Frontend pakai **React (Vite)**.
...
| Grafik | Chart.js via react-chartjs-2 |
...
└── pages/
    ...
    └── Rekap.jsx                -> stat + grafik + ringkasan (owner only)
...
3. Buka **Authentication → Users → Add user**, buat 2 akun:
   - `kasir@toko.com`
   - `owner@toko.com`
4. Copy **UID** masing-masing, lalu jalankan di SQL Editor:
   ```sql
   insert into public.profiles (id, name, role) values
     ('UID-KASIR', 'Kasir Toko', 'kasir'),
     ('UID-OWNER', 'Owner Toko', 'owner');
   ```
5. Buka **Project Settings → API**, copy `Project URL` dan `anon public key`, isi ke `src/lib/supabaseClient.js`:
   ```js
...
6. Install dependency & jalankan:
   ...
   npm run dev
   ```
7. Buka `http://localhost:5173`, login pakai salah satu akun di atas.
